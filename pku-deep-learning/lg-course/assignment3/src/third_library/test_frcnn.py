...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    for key in bboxes:
        ...
        for i in range(new_boxes.shape[0]):
            if new_probs[i] > max_prob:
                max_prob = new_probs[i]
                (x1, y1, x2, y2) = new_boxes[i, :]
                (real_x1, real_y1, real_x2, real_y2) = get_real_coordinates(ratio, x1, y1, x2, y2)
                region = (real_x1, real_y1, real_x2, real_y2)
            all_dets.append((key, 100 * new_probs[i]))
    ...
    if region == None:
        ...
        real_y2 = max(real_y2, 0)
        real_x1 = max(real_x1, 0)
