...
class Attacker(object):
    ...
    def save_samples(self, samples):
        ...
        if not os.path.exists('./adv_train_results/white_box_attack'):
            os.mkdir('./adv_train_results/white_box_attack')
        root_path = './adv_train_results/white_box_attack'
        for idx,sample in enumerate(samples):
            ori_image = sample[0]
            adv_image = sample[1]
            label = sample[2]
...
if __name__ == "__main__":
    ...
    print("[*] White box attack (after adv training)!", flush=True)
    # Load dataset
    d = Fashion_MNIST()
