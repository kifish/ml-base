...
class CNN(object):
    def __init__(self,scope_name = '',is_inference=False,
        ...
        # bulid graph
        (self.__Ylogits,self.__Y,self.__update_ema) = self.__build_graph_version7(self.__X)
    ...
    def __build_graph_version7(self, X):
        self.__W1 = tf.Variable(tf.glorot_uniform_initializer()((5, 5, 1, 64)))
        self.__B1 = tf.Variable(tf.constant(0, self.__dtype, [64]))
        Y1l = tf.nn.conv2d(X, self.__W1,
                           strides=[1, 2, 2, 1],
                           padding='SAME')
        Y1bn, update_ema1 = self.__batchnorm(Y1l, self.__tst, self.__iter,
        ...
        self.__W2 = tf.Variable(tf.glorot_uniform_initializer()((5, 5, 64, 32)))
        self.__B2 = tf.Variable(tf.constant(0, self.__dtype, [32]))
        Y2l = tf.nn.conv2d(Y1d, self.__W2,
                           strides=[1, 2, 2, 1],
                           padding='SAME')
        Y2bn, update_ema2 = self.__batchnorm(Y2l, self.__tst, self.__iter,
                                             self.__B2, convolutional=True)
        # output shape is [None,7,7,32]
        Y2r = tf.nn.relu(Y2bn)
        Y2d = tf.nn.dropout(Y2r,self.__pkeep_conv,
                           self.__compatible_convolutional_noise_shape(Y2r))

        self.__W3 = tf.get_variable('W3',[7 * 7 * 32, 64],initializer=tf.random_normal_initializer())
        self.__B3 = tf.Variable(tf.constant(0, self.__dtype, [64]))
        Y3f = tf.reshape(Y2d, shape=[-1, 7 * 7 * 32])
        Y3l = tf.matmul(Y3f, self.__W3)
        Y3bn, update_ema3 = self.__batchnorm(Y3l, self.__tst,
                                             self.__iter, self.__B3)
        Y3r = tf.nn.relu(Y3bn) #这里的relu非常重要，没有relu这一层的作用就很小了
        Y3d = tf.nn.dropout(Y3r, self.__pkeep)
        self.__W4 = tf.get_variable('W4',[64, 10],initializer=tf.random_normal_initializer())
        self.__B4 = tf.Variable(tf.constant(0, self.__dtype, [10]))
...
#version7:
#改了dropout = 0.95(fc)/0.8(conv) 和batch_size = 128
# [*] Epoch 20/20, Testing start...
# [*] Epoch 20/20, Testing done!
# 	loss = 2.522e-01, acc = 0.914, time = 0.045
#python test.py
