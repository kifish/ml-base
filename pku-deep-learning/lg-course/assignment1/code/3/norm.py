...
# Create the model
weight_scale = 0.1 #0.01,test acc不上升，停留在0.1142; 0.1趋势比较正常，但最终test_acc只有0.8872
# Epoch 179 training loss: 0.31001123969157507, test acc: 0.8872

x = tf.placeholder(tf.float32, [None, 784])
# W_1 = tf.Variable(tf.zeros([784, 128]))
#第一层
W_1 = tf.get_variable(
    'W_1', [784, 100], initializer=tf.random_normal_initializer()) * weight_scale
...
W_2 = tf.get_variable(
    'W_2', [100, 100], initializer=tf.random_normal_initializer()) * weight_scale
...
W_3 = tf.get_variable(
    'W_3', [100, 10], initializer=tf.random_normal_initializer()) * weight_scale
