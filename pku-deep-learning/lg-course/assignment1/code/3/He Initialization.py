...
# 感觉这种初始化效果一开始的epochs比较好，但是最终收敛的效果不如Xavier
...
# Epoch 149 training loss: 0.3385988022486366, test acc: 0.8796
# Create the model
x = tf.placeholder(tf.float32, [None, 784])
# W_1 = tf.Variable(tf.zeros([784, 128]))
#第一层
W_1 = tf.get_variable(
    'W_1', [784, 100], initializer=tf.random_normal_initializer()) / np.sqrt(784/2)
...
W_2 = tf.get_variable(
    'W_2', [100, 100], initializer=tf.random_normal_initializer()) / np.sqrt(100/2)
...
W_3 = tf.get_variable(
    'W_3', [100, 10], initializer=tf.random_normal_initializer()) / np.sqrt(100/2)
