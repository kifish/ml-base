...
# Epoch 149 training loss: 0.2476547600697483, test acc: 0.8912
# Create the model
...
W_3 = tf.get_variable('W_3', [100, 100], initializer=tf.random_normal_initializer())
# b_3 = tf.Variable(tf.zeros([10]))
b_3 = tf.get_variable('b_3', [100], initializer=tf.random_normal_initializer())
a_3 = tf.matmul(z_2, W_3) + b_3
z_3 = tf.nn.relu(a_3)

# 第四层
W_4 = tf.get_variable('W_4', [100, 10], initializer=tf.random_normal_initializer())
b_4 = tf.get_variable('b_4', [10], initializer=tf.random_normal_initializer())
...
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=a_4, labels=y))
train_step = tf.train.GradientDescentOptimizer(0.01).minimize(loss)  # 学习率为0.01

correct_prediction = tf.equal(tf.argmax(a_4, 1), tf.argmax(y, 1))  # (num_training,1),预测正确为1，反之为0
...
with tf.Session() as sess:
    ...
    for y_, cls in enumerate(classes):
        ...
        for i, idx in enumerate(idxs):
            ...
            pred = sess.run(a_4, feed_dict={x: np.array([X_test[idx]])})  # 返回的是numpy.ndarray
