...
# Epoch 149 training loss: 0.27224915196845095, test acc: 0.9224
# Create the model
keep_prob = tf.placeholder("float") #设定为占位符，这样在训练和测试的时候分别控制dropout比例。
#如果模型过拟合不是很严重的话，dropout的比例应该低一些，换言之keep_prob的比例应该高一些，比如0.9
...
z_1 = tf.nn.relu(a_1)
z_1_drop = tf.nn.dropout(z_1,keep_prob)
...
a_2 = tf.matmul(z_1_drop, W_2) + b_2
z_2 = tf.nn.relu(a_2)
z_2_drop = tf.nn.dropout(z_2,keep_prob)
...
a_3 = tf.matmul(z_2_drop, W_3) + b_3
...
with tf.Session() as sess:
    ...
    num_batch = int(num_train/batch_size) + 1  # 整除的情况下会多跑一个batch，不过这影响不大。
    loss_history = []
    val_acc_history = []
    train_acc_history = []
    for epoch in range(num_epoch):
        ...
        for batch_idx in range(num_batch):
            ...
            _, c = sess.run([train_step, loss], feed_dict={
                            x: X_batch, y: y_batch,keep_prob:0.9})
            avg_cost += c / num_batch

        loss_history.append(avg_cost)
        acc = sess.run(accuracy_op, feed_dict={x: X_train, y: y_train,keep_prob:1.0})
    ...
    for y_, cls in enumerate(classes):
        ...
        for i, idx in enumerate(idxs):
            ...
            # 返回的是numpy.ndarray
            pred = sess.run(a_3, feed_dict={x: np.array([X_test[idx]]),keep_prob: 1.0})
