...
class Model():
    ...
    def _pool_layer(self, name, inp, ksize, stride, shape = None,padding='SAME', mode='MAX',version = 'naive'):
        ...
        if mode == 'MAX':
            ...
            else: #按照pytorch里的lp pooling实现
                x = tf.pow(inp,p)
                x = tf.nn.avg_pool(x, ksize=[1, shape[0], shape[0], 1], strides=[1, stride, stride, 1],
                                   padding=padding, name=name, data_format='NHWC')
                x = tf.pow(x,1/p)
        return x
    ...
    def build(self):
        ...
        x = self._pool_layer(name='pool1', inp=x, ksize=None, padding='SAME',shape= [3,3,16,16],stride=2, mode=4) # Nx16x16x16
        ...
        x = self._pool_layer(name='pool2', inp=x, ksize=None, padding='SAME',shape= [4,4,32,32],stride=2, mode=4) # Nx8x8x32
        ...
        x = self._pool_layer(name='pool3', inp=x, ksize=None, padding = 'SAME',shape= [5,5,64,64],stride=2, mode=4) # Nx4x4x64
        ...
        x = self._pool_layer(name='pool4', inp=x, ksize=None, padding='VALID', shape= [4, 4, 128, 128], stride=1, mode=4) # Nx1x1x128
