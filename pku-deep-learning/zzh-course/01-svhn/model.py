...
class Model():
    ...
    def _pool_layer(self, name, inp, ksize, stride, shape = None,padding='SAME', mode='MAX',is_training = False):
        ...
        if mode == 'MAX':
            ...
            with tf.variable_scope(name) as scope:
                ...
                x = tf.pow(x,1/p)
                x = tf.layers.batch_normalization(x, axis=3, training=is_training) #add BN for trial
        return x
    ...
    def build(self):
        ...
        x = self._pool_layer(name='pool1', inp=x, ksize=None, padding='SAME',shape=[3,3,16,16],stride=2, mode=2,is_training = is_training) # Nx16x16x16
        ...
        x = self._pool_layer(name='pool2', inp=x, ksize=None, padding='SAME',shape= [4,4,32,32],stride=2, mode=2,is_training = is_training) # Nx8x8x32
        ...
        x = self._pool_layer(name='pool3', inp=x, ksize=None, padding = 'SAME',shape=[5,5,64,64],stride=2, mode=2,is_training = is_training) # Nx4x4x64
        ...
        x = self._pool_layer(name='pool4', inp=x, ksize=None, padding='VALID', shape=[4, 4, 128, 128], stride=1, mode=2,is_training = is_training) # Nx1x1x128
