...
GRAM_LAYERS = ['conv1_1', 'conv1_2', 'conv2_1', 'conv2_2', 'conv3_1', 'conv3_2',
...
def get_l2_gram_loss_for_layer(noise, source, layer):
    ...
    source_feature_map = tf.transpose(source_feature_map,perms=[0,3,1,2])
    F_target = tf.reshape(source_feature_map, [-1, n_pixel]) # n_channel * n_pixel
    noise_feature_map = tf.transpose(noise_feature_map,perms=[0,3,1,2])
...
def get_gram_loss(noise, source):
    with tf.name_scope('get_gram_loss'):
        gram_loss = [get_l2_gram_loss_for_layer(
            noise, source, layer) for layer in GRAM_LAYERS]
