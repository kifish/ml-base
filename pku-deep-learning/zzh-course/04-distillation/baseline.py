...
def get_dataset_batch(ds_name):
    ...
    ds = tf.data.Dataset.from_generator(
        ds_gnr, output_types=(tf.float32, tf.int32),)
    if ds_name == 'train':
...
def main():
    ...
    assert args.loss in ['softmax', 'abs-max',
                         'square-max', 'plus-one-abs-max', 'non-negative-max']
    ...
    if args.loss == 'softmax':
        ...
        preds = square_logits / \
            tf.reduce_sum(square_logits, axis=1, keepdims=True)
    elif args.loss == 'plus-one-abs-max':
        plus_one_abs_logits = tf.abs(logits) + 1.0
        preds = plus_one_abs_logits / \
            tf.reduce_sum(plus_one_abs_logits, axis=1, keepdims=True)
    ...
    boundaries = [train_set.minibatchs_per_epoch *
                  15, train_set.minibatchs_per_epoch*40]
