...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W1'] = weight_scale * \
        np.random.randn(F, Ch, filter_size, filter_size)
    ...
    self.params['W2'] = weight_scale * np.random.randn(F*Hp*Wp, hidden_dim) #全连接层
  ...
  def loss(self, X, y=None):
    ...
    # (N,Ch,H,W)->conv->(N,F,Hc,Wc)->relu->max-pooling->(N,F,Hp,Wp)
    out, cache = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
    caches.append(cache)
    # ->af,全连接:(F*Hp*Wp,hd)->relu->(N,F*Hp*Wp,hd)
    out, cache = affine_relu_forward(out, W2, b2)
    caches.append(cache)
    # ->af,全连接:(hd,Class)->(N,Class)->softmax->(N,Class)
    #把样本数量N想象为1，更清晰易懂。然后再把样本数量那一维添上即可。
